...
import {UploadMutation} from './mutation/UploadMutation'
...
class Uploads extends Component {
	constructor(props) {
		...
		this.state = {
			...
			currentTitle: 'New Food', currentStep: 1, currentPage: 1, currentEnable: true,
	...
	saveFile = () => {
		...
		.then(function (compressedFile) {
			...
			.then (result => {
				...
				objthis.props.handleProgress(90)
				...
				.then(data => {
					...
					objthis.setState({
						processStatus: 'done',
						currentEnable: false,
						result: parsedData.Labels,
						cover: image.imageInfo
			...
			case 4:
				if (this.state.processStatus == 'done') {
					this.setState({
						currentEnable: true,
						currentPage: 4,
						currentTitle: 'Enter keywords with comma'
					})
				}
				break
			case 5:
			...
			case 6:
				if (this.state.processStatus == 'done') {
					this.setState({
						...
						currentPage: 6,
						resultButton: 'Submit',
						currentTitle: 'Final Step'
					})
				}
				break
			case 7:
				if (this.state.processStatus == 'done') {
					UploadMutation(this.state.name.toLowerCase(), this.state.type.toLowerCase(), new Date(this.state.expiry).getTime().toString(), this.state.keywords.toLowerCase(), this.state.cover).then((response) => {
						if (response.createFood.result.result == 'true') {
							this.props.handleNotification('Successfully added food.')
						} else {
							this.props.handleNotification('Something went wrong!')
						}
					})
					this.setState({
						name: '', type: '', expiry: '', keywords: '', cover: '', result: '', 
						resultButton: 'Next Step', currentTitle: 'New Food', currentStep: 1, currentPage: 1, currentEnable: true, imageInfo: '', imageName: '', imageUrl: '', processStatus: 'pending'
					})
				}
				break
			default:
			...
			<div className={styles.upload_box}>
				...
				<div className={styles.upload_box_content}>
					...
					<div className={styles.upload_box_content_container}>
						...
						<div className={styles.upload_box_content_list}>
							<div className={styles.container}>
								<div className={`${styles.row} ${styles.row_border}`}>
									<div className={`${styles.col_6} ${styles.upload_box_content_list_title}`}>
										Image Name
									</div>
									<div className={`${styles.col_3} ${styles.upload_box_content_list_title}`}>
										Sizes
									</div>
									<div className={`${styles.col_3} ${styles.upload_box_content_list_title}`}>
										Status
					...
					<div className={styles.upload_selectlist}>
						<div className={styles.upload_selectlist_layer}>
							<div className={styles.upload_selectlist_container}>
									{this.state.result != undefined ? this.state.result.map(data => 
										<div className={styles.upload_selectlist_choose} onClick={() => this.handleClick(0, data.Name)}>
											{data.Name}
										</div>
									) : ''}
							</div>
							<div className={styles.upload_inputbox}>
								<input type='text' placeholder="type another name" name="name" value={this.state.name} onChange={this.handleChange} />
							</div>
						</div>
					</div>) : ( this.state.currentPage == 3 ? (
					<div className={styles.upload_selectlist}>
						<div className={styles.upload_selectlist_layer}>
							<div className={styles.upload_selectlist_container}>
									{this.state.result != undefined ? this.state.result.map(data => 
										<div className={styles.upload_selectlist_choose} onClick={() => this.handleClick(1, data.Name)}>
											{data.Name}
										</div>
									) : ''}
							</div>
							<div className={styles.upload_inputbox}>
								<input type='text' placeholder="type another type" name="type" value={this.state.type} onChange={this.handleChange} />
							</div>
						</div>
					</div>) : ( this.state.currentPage == 4 ? (
					<div className={styles.upload_selectlist_layer}>
						<div className={styles.upload_selectlist_container}>
								{this.state.result != undefined ? this.state.result.map(data => 
									<div className={styles.upload_selectlist_choose} onClick={() => this.handleClick(2, data.Name)}>
										{data.Name}
									</div>
								) : ''}
						</div>
						<div className={styles.upload_inputbox}>
							<input type='text' placeholder="apple,fruit,healthy" name="keywords" value={this.state.keywords} onChange={this.handleChange} />
						</div>
					</div>): ( this.state.currentPage == 5 ? (
						<div className={styles.expiry_time}>
							<input type="date" name="expiry" value={this.state.expiry} onChange={this.handleChange} />
						</div>
					):( this.state.currentPage == 6 ? (
						<div className={styles.review_input}>
							<div className={styles.reivew_container}>
				...
				<div className={styles.upload_box_layer}>
